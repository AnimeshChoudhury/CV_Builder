...
class CVBuilder:
    ...
    def _parse_date(self, date_str):
        ...
        # Handle "Current" or "Present" dates
        if 'current' in date_str.lower() or 'present' in date_str.lower():
            # Return a far future date to ensure current positions appear first
    ...
    def _get_end_date(self, period_str):
        """Extract end date from period string for sorting"""
        if not period_str:
    ...
    def _add_experience(self, story):
        ...
        # Sort experience by end date (current onwards - most recent first)
        sorted_experience = sorted(
            self.cv_data.experience,
            key=lambda x: self._get_end_date(x.get('period', '')),
            reverse=True
        )
